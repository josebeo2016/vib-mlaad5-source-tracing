...
class Model(nn.Module):
    ...
    def _forward(self, x):
        ...
        if self.flag_fix_ssl:
            with torch.no_grad():
                x_ssl_feat = self.ssl_model(x.squeeze(-1), is_train = False)
        else:
            x_ssl_feat = self.ssl_model(x.squeeze(-1), is_train = self.is_train) #(bs,frame_number,feat_dim)
